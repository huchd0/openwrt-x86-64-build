...
# ==========================================
# 🚀 8. 终极打包与智能超载拦截机制 (逻辑修正版)
# ==========================================
echo ">>> 🚀 正在以【$BUILD_MODE】模式全速打包固件..."
# 尝试打包，如果报错也继续往下走，我们要人工二次检查体积
make image PROFILE="$FINAL_PROFILE" PACKAGES="$PKGS" FILES="files" || true

# 💡 这里的逻辑改了：不再依赖 make 的返回值，而是直接看 bin 目录下有没有生成文件
# 并且人工校验：只要生成的 bin 文件小于 15600KB (约15.2MB)，就认为它是安全的
FIRMWARE_FILE=$(ls bin/targets/*/*/*.{bin,img.gz} 2>/dev/null | head -n 1)

if [ -f "$FIRMWARE_FILE" ]; then
    FILE_SIZE=$(du -k "$FIRMWARE_FILE" | cut -f1)
    echo ">>> 📊 固件生成成功！当前体积: ${FILE_SIZE}KB"
    
    if [ "$FILE_SIZE" -gt 15800 ]; then
        echo "❌ 警告：固件体积 (${FILE_SIZE}KB) 确实超过了 16MB 物理极限！"
        exit 1
    else
        echo "✅ 体积校验通过（${FILE_SIZE}KB < 16MB），忽略 ImageBuilder 的虚假报错。"
    fi
else
    echo "================================================================"
    echo "❌ 🚨 致命错误：固件未能生成！"
    echo "这说明 ImageBuilder 内部发生崩溃，或选错架构/Profile。"
    echo "================================================================"
    exit 1
fi
